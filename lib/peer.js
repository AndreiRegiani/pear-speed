...
module.exports = class Peer extends EventEmitter {
  ...
  _onconnection(socket, info) {
    ...
    const peer = {
      ...
      port: rawStream ? rawStream.remotePort : 0,
      mode: 'handshake',
      ready: false,
      dropping: false,
      control: b4a.alloc(0),
  ...
  _receiveUpload(peer, data) {
    ...
    this._update()
    this.emit('served', { timestamp: Date.now(), ip: peer.ip, port: peer.port, sent, received })
  }
  ...
  snapshot() {
    ...
    const publicIP = this.swarm.dht.host ? normalizeIP(this.swarm.dht.host) : null
    const publicPort = this.swarm.dht.port || 0
    return {
      phase: this.phase,
      elapsed,
      publicIP: publicIP && publicPort ? `${publicIP}:${publicPort}` : publicIP,
      serving: this.peers
        .filter((peer) => peer.mode.startsWith('serving-'))
        .map((peer) => ({ timestamp: peer.service.startedAt, ip: peer.ip, port: peer.port })),
      peers: peers
